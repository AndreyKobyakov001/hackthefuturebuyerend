...
function isWrongItem(jsonResponse: any, orderDescription: string): boolean {
  ...
  if (
    jsonResponse.order_discrepancies &&
    jsonResponse.order_discrepancies.length > 0
  ) {
    return true;
  }
  ...
  switch (condition_grade) {
    ...
    case "Acceptable":
      conditionDescription =
        "Shows some wear and tear but is still functional.";
      break;
    case "Poor":
...
function generateMarketplaceData(
  jsonResponse: any,
...
function isEligibleForRecycling(
  ...
  const isCriticallyDamaged =
    jsonResponse.damage_severity === "critical failure";
  ...
  return (
    isCriticallyDamaged &&
    isFragileItem &&
    (isDefectiveReason || isShippingOrManufacturingIssue) &&
    !hasSignsOfUse
  );
}
...
OUTPUT AS JSON. INCLUDE ALL FIELDS EVEN IF EMPTY:
{
    ...
    "suggested_price": "$XX.XX"
        ...
        if (forceProceed && wrongItem) {
          ...
          if (wrongItemAttempt) {
            ...
            jsonResponse.comment_analysis.red_flags.push(
              "Item in image doesn't match order description"
            );
        ...
        if (
          !jsonResponse.user_score_adjustment ||
          !jsonResponse.new_user_score
        ) {
        ...
        if (
          (jsonResponse.final_decision === "credit" ||
            jsonResponse.final_decision === "reject") &&
          !jsonResponse.resale_ad
        ) {
          jsonResponse.resale_ad = generateResaleAd(
            jsonResponse,
            orderDescription
          );
        }
        ...
        if (!jsonResponse.marketplace_data) {
          jsonResponse.marketplace_data = generateMarketplaceData(
            jsonResponse,
            orderDescription
          );
        }
    ...
    return NextResponse.json(
      { error: "Failed to process return analysis" },
      { status: 500 }
    );
  }
}
